...
def predict():
    ...
    command = 'python main.py --mode test --data_split dev --para_limit 2250 --batch_size 24 --init_lr 0.1 --keep_prob 1.0 --sp_lambda 1.0 --save HOTPOT-{} --prediction_file dev_distractor_pred.json'.format(save)
    print('running command...\n  {}\n'.format(command))
    os.system(command)
...
def evaluate():
    ...
    command = 'python hotpot_evaluate_v1.py dev_fullwiki_pred.json hotpot_dev_fullwiki_v1.json > ' + result_text
    print('running command...\n  {}\n'.format(command))
    os.system(command)
