...
class Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        if(model_setting[3] == 'L'):
            self.rnn_start = EncoderLSTM(config.hidden+1, config.hidden, 1, False, True, 1-config.keep_prob, False)
        ...
        if(model_setting[4] == 'L'):
            self.rnn_end = EncoderLSTM(config.hidden*3+1, config.hidden, 1, False, True, 1-config.keep_prob, False)
        ...
        if(model_setting[5] == 'L'):
            self.rnn_type = EncoderLSTM(config.hidden*3+1, config.hidden, 1, False, True, 1-config.keep_prob, False)
    ...
    def __init__(self, input_size, num_units, nlayers, concat, bidir, dropout, return_last):
        ...
        for i in range(nlayers):
            if i == 0:
                ...
                input_size_ = num_units if not bidir else num_units * 2
                output_size_ = num_units
            self.rnns.append(nn.LSTM(input_size_, output_size_, 1, bidirectional=bidir, batch_first=True)) # CHANGE!!
        self.rnns = nn.ModuleList(self.rnns)
        self.init_hidden = nn.ParameterList([nn.Parameter(torch.Tensor(2 if bidir else 1, 1, num_units).zero_()) for _ in range(nlayers)])
        self.dropout = LockedDropout(dropout)
        self.concat = concat
