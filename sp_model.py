...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        model_setting = 'RRRRRR'
        print("model_setting: {}".format(model_setting))

        # Changed!!
        if(model_setting[0]=='L'):
            self.rnn = EncoderLSTM(config.char_hidden+self.word_dim, config.hidden, 1, True, True, 1-config.keep_prob, False)
        elif(model_setting[0] == 'R'):
        ...
        # Changed!!
        if(model_setting[1]=='L'):
            self.rnn_2 = EncoderLSTM(config.hidden, config.hidden, 1, False, True, 1-config.keep_prob, False)
        elif(model_setting[1] == 'R'):
        ...
        # Changed!!
        if(model_setting[2]=='L'):
            self.rnn_sp = EncoderLSTM(config.hidden, config.hidden, 1, False, True, 1-config.keep_prob, False)
        elif(model_setting[2] == 'R'):
        ...
        # Changed!!
        if(model_setting[3]=='L'):
            self.rnn_start = EncoderLSTM(config.hidden*3, config.hidden, 1, False, True, 1-config.keep_prob, False)
        elif(model_setting[3] == 'R'):
        ...
        # Changed!!
        if(model_setting[4]=='L'):
            self.rnn_end = EncoderLSTM(config.hidden*3, config.hidden, 1, False, True, 1-config.keep_prob, False)
        elif(model_setting[4] == 'R'):
        ...
        # Changed!!
        if(model_setting[5]=='L'):
            self.rnn_type = EncoderLSTM(config.hidden*3, config.hidden, 1, False, True, 1-config.keep_prob, False)
        elif(model_setting[5] == 'R'):
...
class EncoderRNN(nn.Module):
    ...
    def get_init(self, bsz, i): # make memory for hidden(bidir, bsz, numunits)
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, input_size, num_units, nlayers, concat, bidir, dropout, return_last):
        ...
        for i in range(nlayers):
            if i == 0:
                ...
                input_size_ = num_units if not bidir else num_units * 2
                output_size_ = num_units
            self.rnns.append(nn.LSTM(input_size_, output_size_, 1, bidirectional=bidir, batch_first=True))  # nn.LSTM...init: (RNNBase)
        self.rnns = nn.ModuleList(self.rnns)
        self.init_hidden = nn.ParameterList([nn.Parameter(torch.Tensor(2 if bidir else 1, 1, num_units).zero_()) for _ in range(nlayers)])
        self.dropout = LockedDropout(dropout)
        self.concat = concat
    ...
    def forward(self, input, input_lengths=None):
        ...
        for i in range(self.nlayers):
            hidden = self.get_init(bsz, i)
            cell = self.get_init(bsz, i)
            output = self.dropout(output)
            if input_lengths is not None:
                output = rnn.pack_padded_sequence(output, lens, batch_first=True)
            output, (hidden, cell) = self.rnns[i](output, (hidden, cell))   #------------------------- nn.LSTM...forward:(input, hx=None)  return output,(h_n,c_n)
            if input_lengths is not None:
                output, _ = rnn.pad_packed_sequence(output, batch_first=True)
                if output.size(1) < slen: # used for parallel
                    padding = Variable(output.data.new(1, 1, 1).zero_())
